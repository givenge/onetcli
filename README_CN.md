...
- **MCP 公共工具链** — 完整的 MCP（模型上下文协议）支持：创建连接、执行 SQL、Redis 和 SFTP 操作、类型化工具管理、审批队列、Helper 安装等，全部可作为标准 MCP 工具调用。
- **Redis SSH 隧道** — Redis 连接支持 SSH 隧道，同时修复了搜索时命名空间子项的展示。
- **SFTP MCP 工具** — 远程路径检查、文件上传、文件下载等操作以结构化 MCP 工具形式开放。
- **首页重构** — 连接列表统一渲染，支持拖拽排序和布局切换。
- **窗口置顶** — 支持将 OnetCli 窗口固定在所有窗口之上。
- **SQL 增强** — 查询结果最大行数限制、基于 manifest 的数据库 schema 支持、SQL 编辑器自定义字体导入。
- **终端字体族** — 终端会话支持自定义字体族设置。
- **CLI / 自动化运行时** — 建立统一的 CLI 和自动化工具运行时架构，支持结构化远程操作。
- **AI 修复** — 分离 AI 推理过程与聊天回复，消息展示更清晰。
- **性能优化** — 远程桌面帧传输性能优化。
- **国际化** — 新增 SSH 隧道相关多语言文案。
