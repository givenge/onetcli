...
const TERMINAL_AGENT_OUTPUT_LINES: usize = 160;
const TERMINAL_AGENT_MODEL_MAX_TOKENS: u32 = 1200;
const TERMINAL_AGENT_WAIT_TIMEOUT_SECS: u64 = 120;

const TERMINAL_AGENT_SYSTEM_PROMPT: &str = r#"你是运行在终端中的命令 agent。你不是普通聊天模型，你可以在需要时申请使用工具。

可用工具：
1. `read_terminal_output`
   - 读取当前终端最近输出
   - 参数：`lines`（整数，1-400）
2. `execute_command`
   - 请求执行一条 shell 命令
   - 参数：`command`（字符串）
   - 执行前必须先向用户申请权限；系统会弹出确认框，用户同意后才会真正执行

行为规则：
1. 当问题依赖当前终端状态、错误输出、命令结果时，先调用 `read_terminal_output`，不要猜。
2. 当需要执行命令时，调用 `execute_command`；不要把“请你自己去执行”当成最终答案。
3. 对“查看/检查/列出/统计/搜索/打印/显示”这类可直接执行的查询请求，只要命令明确，就直接调用 `execute_command`，不要先输出说明文字，更不要把命令示例当最终答案。
4. 如果用户要求你“重启/停止/启动/查看/修复”某个进程或服务，只要当前信息足够，就应继续使用工具完成，而不是停在操作说明。
5. 如果要完成一个目标需要多步命令，你应该在多轮中连续调用工具，直到任务完成、明确失败、或确实缺少关键信息。
6. 如果当前信息不足以安全执行下一步，先用工具补充信息；例如查看进程详情、父进程、systemd/service 管理方式、工作目录、启动命令。
7. 只有在任务已经完成、或明确说明为什么当前无法继续自动执行时，才返回 `final`。
8. 不要复述“我应该执行某个命令”。要么真正返回 `execute_command`，要么返回 `read_terminal_output`，要么在已完成后返回 `final`。
9. 每一轮只允许返回一种动作，且必须返回 JSON，不要输出 JSON 以外的文字。
10. 尽量减少不必要的工具调用。
11. 避免产出会进入交互模式或持续阻塞的命令；查看日志类命令优先使用非交互形式，例如 `journalctl --no-pager`。
12. 如果上下文里已经有某条命令的成功执行记录、输出结果、或工具执行历史，不要重复执行相同或等价命令。除非结果为空、命令失败、系统状态可能已变化，或你明确说明为什么必须重新验证。

特别要求：
- 当用户点击“重启这个进程”之类的动作时，不要只返回 kill 或 restart 命令示例；应直接继续申请执行需要的命令。
- 如果重启命令不明确，不要立即结束；先继续调查这个进程是通过哪种方式启动的，再决定执行什么命令。
- 当用户说“查看内存状态”“看磁盘使用情况”“看端口占用”这类请求时，默认这是让你直接执行查询命令，不是让你讲解应该执行什么。

返回格式只能是以下三种之一：
{"type":"final","content":"给用户的最终答复"}
{"type":"read_terminal_output","lines":120,"reason":"为什么需要读取输出"}
{"type":"execute_command","command":"ls -la","reason":"为什么要执行这条命令"}
"#;

#[derive(Debug, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
enum TerminalAgentDecision {
    Final {
        content: String,
    },
    ReadTerminalOutput {
        #[serde(default = "default_terminal_agent_lines")]
        lines: usize,
        #[serde(default)]
...
struct PendingAgentCommand {
    command: String,
    before_output: String,
    responder: oneshot::Sender<TerminalAgentCommandResult>,
}

struct TerminalAgentCommandResult {
    exit_code: Option<i32>,
    output: String,
}

fn default_terminal_agent_lines() -> usize {
    120
}

fn parse_terminal_agent_decision(
    response: &str,
) -> Result<TerminalAgentDecision, serde_json::Error> {
    let trimmed = response.trim();
    if let Ok(parsed) = serde_json::from_str::<TerminalAgentDecision>(trimmed) {
        return Ok(parsed);
    }

    if let Some(start) = trimmed.find('{') {
        if let Some(end) = trimmed[start..].rfind('}') {
            if let Ok(parsed) = serde_json::from_str(&trimmed[start..=start + end]) {
                return Ok(parsed);
            }
        }
    }
            ...
            '}' => {
                ...
                if depth == 0 {
                    if let Some(candidate_start) = start.take() {
                        candidates.push(&input[candidate_start..=index]);
                    }
                }
            }
            _ => {}
...
fn terminal_agent_protocol_error_message() -> String {
    "终端助手返回了不符合协议的内容，已中止本轮自动执行。请重试；若持续出现，请检查对应模型是否稳定支持严格 JSON 输出。".to_string()
}
...
fn prepend_terminal_agent_system_prompt(messages: Vec<Message>) -> Vec<Message> {
    let mut merged_system_parts = vec![TERMINAL_AGENT_SYSTEM_PROMPT.to_string()];
    ...
    let mut result = vec![Message::text(
        Role::System,
        merged_system_parts.join("\n\n"),
    )];
    result.extend(rest);
    result
}

fn append_terminal_agent_tool_result(
    messages: &mut Vec<Message>,
    assistant_response: String,
    tool_name: &str,
    tool_result: String,
) {
    messages.push(Message::text(Role::Assistant, assistant_response));
    messages.push(Message::text(
        Role::User,
        format!(
            "Tool `{tool_name}` result:\n```text\n{tool_result}\n```\n请基于这个结果继续，只返回下一步 JSON。"
        ),
    ));
}
...
fn diff_terminal_output(before: &str, after: &str) -> String {
    ...
    for candidate in (0..=max_overlap).rev() {
        if before_lines[before_lines.len().saturating_sub(candidate)..] == after_lines[..candidate]
        {
            overlap = candidate;
            break;
        }
...
fn extract_command_output(before: &str, after: &str, command: &str) -> String {
    let diff = diff_terminal_output(before, after);
    let mut lines: Vec<&str> = diff.lines().collect();
    let command = command.trim();

    if let Some(index) = lines.iter().rposition(|line| line.contains(command)) {
        lines = lines.split_off(index + 1);
    } else {
        while lines
            .first()
...
fn prepare_terminal_agent_command(command: &str) -> String {
    ...
    format!("SYSTEMD_PAGER=cat SYSTEMD_LESS=FRX PAGER=cat LESS=FRX {trimmed}")
}

async fn collect_terminal_agent_response(
    provider: &dyn one_core::llm::LlmProvider,
    request: &ChatRequest,
    cancel_token: &CancellationToken,
    this: &WeakEntity<TerminalView>,
    cx: &mut AsyncApp,
    ...
    let thinking_message_id = format!("{assistant_message_id}-thinking-{step_index}");
                    ...
                    Some(Ok(response)) => {
                        if let Some(thinking) = extract_stream_reasoning(&response) {
                            ...
                            if let Some(entity) = this.upgrade() {
                                let content = full_thinking.clone();
                                let msg_id = thinking_message_id.clone();
                                let _ = cx.update(|cx| {
                                    entity.update(cx, |view, cx| {
                                        view.sidebar.update(cx, |sidebar, cx| {
                                            sidebar.update_agent_thinking(&msg_id, content.clone(), cx);
                                        });
                                    });
                                });
                            }
                        ...
                        let is_done = response.choices.iter().any(|choice| {
                            choice.finish_reason.as_ref().map(|reason| reason != "null").unwrap_or(false)
                        });
                            ...
                            if !full_thinking.is_empty() {
                                if let Some(entity) = this.upgrade() {
                                    ...
                                    let _ = cx.update(|cx| {
                                        entity.update(cx, |view, cx| {
                                            view.sidebar.update(cx, |sidebar, cx| {
                                                sidebar.finish_agent_thinking(&msg_id, cx);
                                            });
                                        });
                                    });
                                }
...
fn truncate_agent_history_output(output: &str, max_chars: usize) -> String {
...
pub struct TerminalView {
    ...
    /// 最近一次 agent 请求绑定的窗口句柄
    agent_window_handle: Option<AnyWindowHandle>,
    /// agent 等待中的命令执行回调
    pending_agent_command: Option<PendingAgentCommand>,
...
impl TerminalView {
    ...
    fn handle_sidebar_event(
        ...
        match event {
            ...
            TerminalSidebarEvent::AiSubmit(content) => {
                self.start_terminal_agent_run(content.clone(), window, cx);
            }
    ...
    fn handle_terminal_event(
        ...
        match event {
            ...
            TerminalModelEvent::WorkingDirChanged(path) => {
                let path = path.clone();
                self.sidebar.update(cx, |sidebar, cx| {
                    sidebar.set_ai_working_dir(Some(path.clone()), cx);
                    sidebar.set_file_manager_initial_dir(path.clone(), cx);
                    sidebar.sync_file_manager_path(path, cx);
                });
            }
            TerminalModelEvent::CommandFinished(exit_code) => {
                debug!(
                    target: "terminal.agent",
                    exit_code = *exit_code,
                    has_pending = self.pending_agent_command.is_some(),
    ...
    pub fn set_font_family(&mut self, family: impl Into<SharedString>, cx: &mut Context<Self>) {
        let family: SharedString = family.into();
        if self.font_family == family {
            return;
        }
        self.font_family = family.clone();
        let persisted_family = family.to_string();
        let _ = update_settings(cx, move |settings| {
            settings.font_family = persisted_family;
        });
        cx.notify();
    }
    ...
    fn start_terminal_agent_run(
        &mut self,
        content: String,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.agent_window_handle = cx.active_window();
        info!(
            target: "terminal.agent",
            has_window = self.agent_window_handle.is_some(),
            input = %content,
            "starting terminal agent run"
        );
        let cancel_token = CancellationToken::new();
        let request = match self.sidebar.update(cx, |sidebar, cx| {
            sidebar.begin_agent_request(content.clone(), cancel_token.clone(), cx)
        }) {
            Ok(request) => request,
            Err(err) if err == "request_ignored" => {
                debug!(
                    target: "terminal.agent",
                    "terminal agent request ignored"
                );
                return;
            }
            Err(err) => {
                warn!(
                    target: "terminal.agent",
                    error = %err,
                    "failed to begin terminal agent request"
                );
                return;
            }
        };

        let global_provider_state = cx.global::<one_core::llm::GlobalProviderState>().clone();
        ...
        let mut messages = prepend_terminal_agent_system_prompt(request.messages.clone());
        debug!(
            target: "terminal.agent",
            assistant_message_id = %assistant_message_id,
            session_id = ?session_id,
            message_count = messages.len(),
            "terminal agent request prepared"
        );
        ...
        cx.spawn(async move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
                ...
                .await
            {
                ...
                Err(error) => {
                    warn!(
                        target: "terminal.agent",
                        error = %error,
                        "failed to resolve provider for terminal agent"
                    );
                    if let Some(entity) = this.upgrade() {
                        let _ = cx.update(|cx| {
                            entity.update(cx, |view, cx| {
                                view.sidebar.update(cx, |sidebar, cx| {
                                    sidebar.fail_agent_request(
                                        &assistant_message_id,
                                        format!("Failed to get provider: {error}"),
                                        cx,
                                    );
                                });
                            });
            ...
            loop {
                if cancel_token.is_cancelled() {
                    info!(
                        target: "terminal.agent",
                        "terminal agent run cancelled before next step"
                    );
                    if let Some(entity) = this.upgrade() {
                        let _ = cx.update(|cx| {
                            entity.update(cx, |view, cx| {
                                view.sidebar.update(cx, |sidebar, cx| {
                                    sidebar.finish_agent_cancelled(cx);
                                });
                            });
                        });
                    }
                ...
                let request = ChatRequest {
                    ...
                    max_tokens: Some(TERMINAL_AGENT_MODEL_MAX_TOKENS),
                ...
                let response: String = match collect_terminal_agent_response(
                    provider.as_ref(),
                    &request,
                    &cancel_token,
                    &this,
                ...
                .await {
                    ...
                    Err(error) if error == "cancelled" => {
                        info!(
                            target: "terminal.agent",
                            "terminal agent run cancelled during provider call"
                        );
                        if let Some(entity) = this.upgrade() {
                            let _ = cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                    ...
                    Err(error) => {
                        warn!(
                            target: "terminal.agent",
                            error = %error,
                            "provider call failed during terminal agent run"
                        );
                        if let Some(entity) = this.upgrade() {
                            let _ = cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                ...
                debug!(
                    target: "terminal.agent",
                    response = %response,
                    "terminal agent raw decision"
                );
                match parse_terminal_agent_decision(&response) {
                    Ok(TerminalAgentDecision::Final { content }) => {
                        info!(
                            target: "terminal.agent",
                            "terminal agent produced final answer"
                        );
                        if let Some(entity) = this.upgrade() {
                            info!(
                                target: "terminal.agent",
                                content_len = content.len(),
                                "terminal agent run completed"
                            );
                            let _ = cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                                    view.sidebar.update(cx, |sidebar, cx| {
                    ...
                    Ok(TerminalAgentDecision::ReadTerminalOutput { lines, reason }) => {
                        invalid_decision_count = 0;
                        info!(
                            target: "terminal.agent",
                            lines,
                            reason = ?reason,
                            "terminal agent requested terminal output"
                        );
                        let status = reason.unwrap_or_else(|| "正在读取终端输出...".to_string());
                        if let Some(entity) = this.upgrade() {
                            let msg_id = assistant_message_id.clone();
                            let _ = cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                                    view.sidebar.update(cx, |sidebar, cx| {
                                        sidebar.set_agent_status(&msg_id, status.clone(), cx);
                                        sidebar.append_agent_history(
                                            &msg_id,
                                            format!("已读取终端最近 {lines} 行输出。"),
                                            cx,
                                        );
                                    });
                                });
                        ...
                        let tool_result = if let Some(entity) = this.upgrade() {
                            cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                                    view.read_terminal_output(lines.clamp(1, 400), cx)
                                })
                            })
                        } else {
                            "Terminal view is unavailable".to_string()
                        };
                        debug!(
                            target: "terminal.agent",
                            output_len = tool_result.len(),
                            "terminal output tool returned"
                        );

                        append_terminal_agent_tool_result(
                            &mut messages,
                            response.clone(),
                    ...
                    Ok(TerminalAgentDecision::ExecuteCommand { command, reason }) => {
                        ...
                        info!(
                            ...
                            command = %command,
                            reason = ?reason,
                            "terminal agent requested command execution"
                        );
                        let status = reason
                            .clone()
                            .unwrap_or_else(|| "等待用户授权执行命令...".to_string());
                        if let Some(entity) = this.upgrade() {
                            let msg_id = assistant_message_id.clone();
                            let _ = cx.update(|cx| {
                                entity.update(cx, |view, cx| {
                        ...
                        let approved = match Self::request_terminal_command_approval(
                            this.clone(),
                            cx,
                            command.clone(),
                            reason.clone(),
                        ...
                        .await
                        {
                            ...
                            Err(error) => {
                                warn!(
                                    ...
                                    error = %error,
                                    command = %command,
                                    "terminal command approval request failed"
                                );
                                if let Some(entity) = this.upgrade() {
                                    let _ = cx.update(|cx| {
                                        entity.update(cx, |view, cx| {
                        ...
                        info!(
                            ...
                            command = %command,
                            approved,
                            "terminal command approval resolved"
                        );
                        let tool_result = if approved {
                            ...
                            match Self::execute_terminal_agent_command(
                                this.clone(),
                                cx,
                                command.clone(),
                            )
                            .await
                            {
                                Ok(result) => {
                                    info!(
                                        ...
                                        "terminal command execution finished"
                                    );
                                    let history_output =
                                        truncate_agent_history_output(&result.output, 1800);
                                    if let Some(entity) = this.upgrade() {
                                        let msg_id = assistant_message_id.clone();
                            ...
                            "Tool `execute_command` was denied by the user.".to_string()
                        };
                    ...
                    Err(_) => {
                        ...
                        messages.push(Message::text(
                            Role::User,
                            "你上一轮没有按协议返回合法 JSON。不要重复上一轮内容，不要解释，不要输出命令示例，不要输出代码块，也不要复述协议。请丢弃上一轮回答，严格只返回以下三种 JSON 之一：{\"type\":\"final\",...} / {\"type\":\"read_terminal_output\",...} / {\"type\":\"execute_command\",...}。对于可直接执行的查看类请求，优先返回 execute_command。".to_string(),
                        ));
                    }
                }
            }
    ...
    fn read_terminal_output(&self, lines: usize, cx: &Context<Self>) -> String {
        debug!(
            target: "terminal.agent",
            lines,
            "reading recent terminal output"
        );
        self.terminal.read(cx).recent_output(lines)
    }

    async fn request_terminal_command_approval(
        ...
        let reason = reason.unwrap_or_else(|| "AI 需要执行一条终端命令".to_string());

        let _ = cx.update(|cx| {
            let window_id = if let Some(entity) = this.upgrade() {
                entity.update(cx, |view, cx| {
                    view.agent_window_handle.or_else(|| cx.active_window())
                })
            } else {
                None
            };
            ...
            let Some(window_id) = window_id else {
                warn!(
                    target: "terminal.agent",
                    command = %command,
                    "no window available for command approval dialog"
                );
                let _ = tx.send(false);
                return;
            };
            debug!(
                ...
                command = %command,
                reason = %reason,
                "opening terminal command approval dialog"
            );

            let tx = Rc::new(RefCell::new(Some(tx)));
            let command_confirm = command.clone();
            let reason_confirm = reason.clone();
            ...
            let _ = cx.update_window(window_id, |_, window, cx| {
                ...
                window.open_dialog(cx, move |dialog, _window, _cx| {
                    ...
                    let muted = muted;
                    let command_for_ok = command_confirm.clone();
                    let command_for_cancel = command_confirm.clone();
                    ...
                    dialog
                        ...
                        .on_ok(move |_, _, _| {
                            info!(
                                target: "terminal.agent",
                                command = %command_for_ok,
                                "user approved terminal command"
                            );
                        ...
                        .child(
                            v_flex()
                                .gap_2()
                                .child(div().text_sm().child(reason_confirm.clone()))
                                .child(
                                    div()
                                        .rounded_md()
                                        .bg(muted)
    ...
    async fn execute_terminal_agent_command(
        ...
        cx: &mut AsyncApp,
        command: String,
    ) -> Result<TerminalAgentCommandResult, String> {
        let (tx, rx) = oneshot::channel();
        let sanitized = prepare_terminal_agent_command(&command);
        debug!(
            target: "terminal.agent",
            command = %sanitized,
            "scheduling terminal command execution"
        );

        let _ = cx.update(|cx| {
            if let Some(entity) = this.upgrade() {
                entity.update(cx, |view, cx| {
                    if view.pending_agent_command.is_some() {
                        warn!(
                            target: "terminal.agent",
                            command = %sanitized,
                            "pending terminal agent command already exists"
                        );
                        let _ = tx.send(TerminalAgentCommandResult {
                            exit_code: None,
                            output: "Another agent command is already running.".to_string(),
                        });
                        return;
                    }

                    let before_output = view
                        .terminal
                        .read(cx)
                        .recent_output(TERMINAL_AGENT_OUTPUT_LINES);
                    view.pending_agent_command = Some(PendingAgentCommand {
                    ...
                    info!(
                        target: "terminal.agent",
                        command = %sanitized,
                        "writing terminal command to PTY"
                    );
                    view.write_to_pty(format!("{sanitized}\n").into_bytes(), cx);
                });
            } else {
                warn!(
                    target: "terminal.agent",
                    command = %sanitized,
                    "terminal view unavailable while executing command"
                );
                let _ = tx.send(TerminalAgentCommandResult {
                    exit_code: None,
                    output: "Terminal view is unavailable.".to_string(),
                });
            }
        });

        let result =
            tokio::time::timeout(Duration::from_secs(TERMINAL_AGENT_WAIT_TIMEOUT_SECS), rx)
                .await
                .map_err(|_| "Command execution timed out".to_string())?
                .map_err(|_| "Command execution channel closed".to_string());
        ...
        match &result {
            Ok(done) => {
                info!(
                    ...
                    output_len = done.output.len(),
                    "terminal command completion signal received"
                );
            }
            Err(error) => {
                warn!(
...
mod tests {
    ...
    #[test]
    fn parse_terminal_agent_decision_extracts_last_valid_json_object() {
        let response = r#"{
...

{
  ...
  "reason": "获取网络接口配置信息"
        ...
        let decision = parse_terminal_agent_decision(response).expect("should parse");
        match decision {
            TerminalAgentDecision::ExecuteCommand { command, reason } => {
                assert_eq!(command, "ip a");
                assert_eq!(reason.as_deref(), Some("获取网络接口配置信息"));
            }
            _ => panic!("expected execute_command"),
        }
    }

    #[test]
    fn json_object_candidates_handles_braces_inside_strings() {
        let input = r#"prefix {"type":"final","content":"value with { braces } inside"} suffix"#;
        let candidates = json_object_candidates(input);
        assert_eq!(candidates.len(), 1);
        assert_eq!(
            candidates[0],
            r#"{"type":"final","content":"value with { braces } inside"}"#
        );
    }

    #[test]
    fn terminal_agent_protocol_error_message_is_user_safe() {
        let message = terminal_agent_protocol_error_message();
        assert!(message.contains("不符合协议"));
        assert!(!message.contains("We are not in danger"));
    }
